...
`undecided` is the level that matters. §3.3 does not say "escalate on breach" and stop; it
...
def working_days_between(a, b):
    ...
    §3.3 says five *working* days. Counting calendar days would silently shorten the window
            ...
            if now > due:
                ...
                if dec:
                    ...
                    detail.append(f"{clock} breached {overdue_wd} working days ago and no "
                                  f"decision is recorded — §3.3 required one within {decision_window}")
                else:
