...
`undecided` is the level that matters. WI §7 #6 does not say "escalate on breach" and stop; it
...
def working_days_between(a, b):
    ...
    WI §7 #6 says five *working* days. Counting calendar days would silently shorten the window
...
def main():
    ...
    ap.add_argument("--units", help="group-remediation.py output. With it, escalation happens "
                                    ...
                                    "the whole point of WI §6.3: one decision, not hundreds")
            ...
            if now > due:
                ...
                if dec:
                    ...
                    detail.append(f"{clock} breached {overdue_wd} working days ago and no "
                                  f"decision is recorded — WI §7 #6 required one within {decision_window}")
                else:
    ...
    # --- collapse to remediation actions (WI §6.3) ------------------------------
