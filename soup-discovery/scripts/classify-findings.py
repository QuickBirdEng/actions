#!/usr/bin/env python3
"""Apply the classification (WI §5, WI §6) to the findings in a BOM.
...
def classify(v, policy):
    """Return (track, rule, why). Rules are WI §5, first match wins."""
    p = props(v)
...
def main():
    ...
    # --- SOUP records whose grq-4 snapshot no longer matches reality (WI §7 #4) ------
    ...
    # High findings is one record to look at, not 40 alerts — the same reason WI §6.3 attaches
    ...
    for v in bom.get("vulnerabilities", []) or []:
        ...
        if was and was.get("track") in TRACK_ORDER:
            # WI §5 — a track may only move up. EPSS is recomputed daily and decays; without
        ...
        # The clock starts when we first saw it (WI §6), not at CVE publication. An escalation
        ...
        if baseline:
            out["baseline"] = {
                ...
                "why": ("present when monitoring began, so its deadlines run from the agreed "
                        "baseline date rather than from the first scan (WI §4.5). Recorded, not "
                        "waived — the finding keeps its track."),
            }
        if latched_from:
            out["latched"] = {"from": latched_from, "held_at": track,
                              "why": "a track may only move up (WI §5)"}
        ...
        for clock in ("mitigation", "remediation"):
            ...
            if delta is None:
                ...
                if spec == "next-release":
                    # WI §6.2: the next maintenance window, not a date derived from an observed
    ...
    doc = {
        ...
        # WI §7 #4 — records whose grq-4 snapshot no longer matches what the scan finds. Not an
