...
verify-bom-against-lockfile.sh| Proves a supplied CycloneDX document describes the closure the Gradle lockfile pins, compared on `group:artifact:version`. Resolving an Android closure needs more than Gradle: without `flutter pub get` the same command resolves a strict subset — measured, 49 of 153 — with no error. The lockfile is the contract; a BOM that does not match it is refused.  
prepare-supplied-bom.sh| A candidate whose closure cannot be read off its artefact — syft finds no components in an AAB, dex bytecode carries no package metadata — may instead be given a CycloneDX document through `SBOM_ARTIFACT_<id>`. Refuses anything that is not CycloneDX, and drops components carrying `project_path`: those are the build's own subprojects, which the manifest already names. Normalisation and the BOM gate still run, so a supplied BOM is held to the same bar as a scanned one.  
