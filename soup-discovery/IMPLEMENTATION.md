...
prepare-supplied-bom.sh| A CycloneDX document given through `SBOM_ARTIFACT_<id>` is used for that candidate in place of whatever discovery found — it describes the same closure from the build that produced it, which is more than a scan can say. Refuses anything that is not CycloneDX and drops `project_path` subprojects. Normalisation and the BOM gate still run.  
